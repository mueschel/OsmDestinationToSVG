...
$colorcodes->{AT}{'blue'}      = '2568aa';
$colorcodes->{AT}{'yellow'}    = 'ffed00';
$colorcodes->{AT}{'green'}     = '12623f'; ##007f00

$colorcodes->{DE}{'blue'}      = '2568aa';
...
$colorcodes->{FR}{'brown'}     = '826500';
$colorcodes->{FR}{'red'}       = 'e41f13';
$colorcodes->{FR}{'lightblue'} = '2a7fff';

$colorcodes->{GR}{'blue'}      = '2568aa';
...
$colorcodes->{SR}{'blue'}      = '2568aa';
$colorcodes->{SR}{'yellow'}    = 'f0e060';
...
sub getRGBColor {
  ...
  if (my ($ct,$cl) = $col =~ /^([A-Z]{2}):([a-zA-Z]+)$/) {
    $cl = lc($cl);
    if (defined $colorcodes->{$ct}{$cl}) {
      return '#'.$colorcodes->{$ct}{$cl};
      }
    $col = $cl;
    }

  $col = lc($col);
  if (defined  $colorcodes->{$conf->{country}}{$col}) {
    return '#'.$colorcodes->{$conf->{country}}{$col};
    }

  if (defined  $colorcodes->{$col}) {
    return '#'.$colorcodes->{$col};
    }
